...
def extract_hospital_features(data_series: pd.Series, argument: str) -> pd.Series:
    ...
    if feature_type_from_arg == "pow":
        ...
        for idx_name in data_series.index:
            if not isinstance(idx_name, str): continue
...
def plot_electrode_pairs(
        ...
        vmin: float,
        vmax: float,
        channel_pos: dict = CHANNEL_POS_19,
        node_size: int = 1200,
...
def plot_eeg_feature_overview(
    ...
    for i, (f_low, f_high) in enumerate(freq_bands):
        ...
        if r >= nrows: break
        ax = axs[r, c]
        band_str = f"{int(float(f_low))}_{int(float(f_high))}"
        series = extract_hospital_features(features_series, f"coh_{band_str}")
    ...
    r, c = divmod(plot_idx, ncols)
    axs[r, c].axis('off')  # Add empty plot for spacing
    plot_idx += 1
    ...
    for i, (f_low, f_high) in enumerate(freq_bands):
        ...
        ax = axs[r, c]
        band_str = f"{int(float(f_low))}_{int(float(f_high))}"
        series = extract_hospital_features(features_series, f"pow_{band_str}")
    ...
    if r < nrows:
        ax = axs[r, c]
        series = extract_hospital_features(features_series, "cov")
        plot_electrode_pairs(
            series, ax, "Cov", channels, vmin=vmin_global, vmax=vmax_global
        )
        plot_idx += 1
    ...
    fig.suptitle(figure_title, fontsize=44)
